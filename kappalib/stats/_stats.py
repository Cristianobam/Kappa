...
def ttest(x, y = None, alternative = "two-sided", mu = 0, paired = False, var_equal = True, alpha = 0.95):
    """
    ...
    try:
        ...
        pvalue = {'two-sided':(1.0 - t.cdf(abs(statistic), v)) * 2.0,
                'less':(1.0 - t.cdf(abs(statistic), v)),
                'bigger':t.cdf(abs(statistic), v)}
            
        summary = Summary({'test':test, 'statistic':statistic, 'pvalue':pvalue[alternative](statistic,v),
                         'df':v, 'xmean':x.mean(), 'ymean':y.mean()})
        summary.summary()
        return summary
